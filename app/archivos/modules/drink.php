...
switch ( $_REQUEST['act'] ) {
	...
	case 'registro':
		...
		if(!$verificar['id']) {
			$db->query( "INSERT INTO usuarios_drink2nite (nombre, apellido, correo, password, nacimiento, sexo, registro) values ('{$nombre}','{$apellido}','{$email}','{$password}','{$fecha}', '{$sexo}', '".time()."')" );
			$accion = 0;
	...
	function generarCodigo($longitud) { 
		...
		$email = $_REQUEST['email'];
		$verificar = $db->super_query( "SELECT id, nombre FROM usuarios_drink2nite WHERE correo = '".$email."'");
...
<div align="center" style="margin-top:20px;"><img src="http://www.drink2nite.com/tema/Defecto/images/logo.png" width="200"></div>
...
Estimado <b style="color:#666;">'.$verificar['nombre'].'</b>, hemos recibido tu solicitud para poder restablecer la contraseña de tu cuenta para que puedas seguir gozando de nuestros servicios. <br><br>El código es: <b style="color:#666;">'.$codigo2.'</b><br><br>Gracias por preferirnos<br><br>Atentamente, el equipo de drink2nite<span style="display:block; margin:10px 0 0 0; border-top:1px solid #DDD; padding:10px 0 0 0; font-size:10px; color:#999; text-align:center;">Este correo es enviado para '.$email.'<br><b style="color:#666;">Importante:</b> Por favor, no responder o hacer reply a este correo electrónico.</span>
</div><div class="cop">©'.date('Y',time()).' drink2nite.com | Todos los derechos reservados</div>
</body>';
			$mensaje = '<html><head><title>Recuperar</title></head>'.$mensaje.'</html>';
			mail($email,$asunto,$mensaje,$headers);
		}
		
		$accion = array("mensaje" => $accion, "generado" => $codigo);
		$accion = $_GET['callback'].'('.json_encode($accion).')';
	break;
	...
	$foto = "http://www.drink2nite.com/subidas/{$lug}{$file_name}";
	return $foto;
	}
	...
	enviar_push($row2['nombre'],$row['nombre'].' acaba de realizar un venue en este local',''.$daten.'');
			}
}
		$accion = $idv;
	...
	while ( $row = $db->get_row($buscar_db) ) {
		if($row['logo']) { $logo = 'http://drink2nite.com/subidas/logos/'.$row['logo']; }
		...
		$resultados .= '<div class="sugerencias_item" onclick="local(\''.$row['id'].'\', \''.$row['local'].'\')"><div class="logo"><img src="'.$logo.'"></div><div class="texto"><a>'.$row['nombre'].'</a><div class="categorias">'.$tipo.'<span>•</span>De '.$row['h_e'].' a las '.$row['h_c'].'<span>•</span>'.$estado.'</div><div class="direccion">'.$row['direccion'].'</div></div></div>';
	}
	...
	*/
	...
	$val_db = $db->super_query( "SELECT COUNT(valoracion) as cantidad FROM valoracion WHERE local = '".$_REQUEST['id']."'");
	$cuenta = $val_db['cantidad'];
	...
	while ( $row2 = $db->get_row($publicaciones_db) ) {
		...
		if($row2['imagen']) $imagen = '<img src="http://drink2nite.com/subidas/locales/'.$row2['imagen'].'" class="imagen">'; else $imagen = '';
...
$promociones_db = $db->query( "SELECT imagen FROM promos WHERE id_l = '".$_REQUEST['id']."' AND estado = '1' ORDER by id DESC LIMIT 0,3");
	while ( $row3 = $db->get_row($promociones_db) ) {
		$promo .= '<ons-carousel-item><img src="http://drink2nite.com/subidas/promos/'.$row3['imagen'].'" style="width: 100%; border-radius:3px;"> </ons-carousel-item>';
	}
	...
	if($file) { $imagen = '<img src="http://drink2nite.com/subidas/locales/'.$archivo.'" class="imagen">'; }
	...
	if($row['seguidores']) {
		...
		$list = explode( ",", $row['seguidores'] );
			foreach ( $list as $daten ) {
				...
				enviar_push('Check-in',$_POST['usuario_nombre'].' acaba de realizar check-in en '.$row2['nombre'],''.$daten.'');
			}
}
	break;
	...
	$id = $_POST['local_id2'];
	$texto = $_POST['texto2'];
	if($file) { $imagen = '<img src="http://drink2nite.com/subidas/eventos/'.$archivo.'" class="imagen">'; }
...
if(!$publicaciones) { $publicaciones = '<div style="padding:20px; text-align:center;">No se encontraron publicaciones</div>'; } else { $publicaciones = $publicaciones;}
		...
		$accion = array("nombre" => $row['nombre'], "apellido" => $row['apellido'], "correo" => $row['correo'], "foto" => $foto, "botones" => $botones, "seguidos" => $row['seguidos_n'], "seguidores" => $row['seguidores_n'], "check" => $cuenta, "publicaciones" => $publicaciones);
	} 
	$accion = $_GET['callback'].'('.json_encode($accion).')';
	break;
...
$eventos_db = $db->query( "SELECT * FROM eventos WHERE id_u IN (".$in.") AND tipo = '2' ORDER by id DESC LIMIT ".$total.",".$pagina."");
	while ( $row2 = $db->get_row($eventos_db) ) {
		...
		if($row2['imagen']) $imagen = '<img src="http://drink2nite.com/subidas/eventos/'.$row2['imagen'].'" class="imagen">'; else $imagen = '';
		if($local['logo']) { $logo = 'http://drink2nite.com/subidas/logos/'.$local['logo']; } else { $logo = 'images/avatar.png'; }
...
function getExtension($str) {
	...
	case 'locales_cerca':
		...
		$busqueda = $db->query( "SELECT (6371 * ACOS( 
					   ...
					   ) AS distance, nombre, logo, lugar, id FROM locales ORDER BY distance ASC LIMIT 0,10" );
...
case 'tonightv2':
...
) AS distance, usuarios_drink2nite.id, CONCAT(usuarios_drink2nite.nombre, ' ' ,usuarios_drink2nite.apellido) AS completo, notificaciones.venue, notificaciones.respuesta, notificaciones.fecha, notificaciones.tipo, locales.logo, locales.nombre, locales.id FROM notificaciones INNER JOIN usuarios_drink2nite ON usuarios_drink2nite.id = notificaciones.propietario INNER JOIN locales ON locales.id = notificaciones.local WHERE notificaciones.user = '".$id."' ORDER BY fecha DESC LIMIT 0,10" );
	...
	if($uid != $row['usuario'] and $tipo != 4) {
		...
		enviar_push($row['nombre'],$row2['nombre'].' acaba de confirmar en tu venue de este local',''.$row['usuario'].'');
	}
	...
	if($row2['seguidores']) {
		$list = explode( ",", $row2['seguidores'] );
			foreach ( $list as $daten ) {
				...
				$db->query("UPDATE usuarios_drink2nite set notificacion=notificacion+1 where id='".$daten."'");
				enviar_push($row3['nombre'],$row2['nombre'].' acaba de escribir en el chat del venue de este local',''.$daten.'');
			}
	}
	if($u == 0 and $id_u != $row3['usuario']) {
		enviar_push($row3['nombre'],$row2['nombre'].' acaba de escribir en tu venue de este local',''.$row3['usuario'].'');
	}
	$accion = json_encode($someArray);
	break;
