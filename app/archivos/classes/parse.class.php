...
class ParseFilter {
	...
	function BB_Parse($source, $use_html = TRUE) {
		...
		$source = preg_replace( "#\[(izquierda|derecha|centro)\](.+?)\[/\\1\]#is", "<div class=\"\\1\">\\2</div>", $source );

		$source = str_ireplace( "[b]", "<b>", str_replace( "[/b]", "</b>", $source ) );
		$source = str_ireplace( "[i]", "<i>", str_replace( "[/i]", "</i>", $source ) );
		$source = str_ireplace( "[u]", "<u>", str_replace( "[/u]", "</u>", $source ) );
		$source = str_ireplace( "[s]", "<s>", str_replace( "[/s]", "</s>", $source ) );		
	...
	function build_media($url) {
		...
		if ($source['host'] != "youtube.com" AND $source['host'] != "rutube.ru" AND $source['host'] != "vimeo.com" AND $source['host'] != "video.mail.ru" AND $source['host'] != "smotri.com") return "[media=".$url."]";

		if ($source['host'] == "youtube.com" OR $source['host'] == "rutube.ru") {
		...
		if ($source['host'] == "youtube.com") {
			...
			} else return '<!--dle_media_begin:'.$decode_url.'--><iframe title="YouTube video player" width="'.$width.'" height="'.$height.'" src="http://www.youtube.com/embed/'.$video_link.'?rel='.intval($this->video_config['tube_related']).'" frameborder="0" allowfullscreen></iframe><!--dle_media_end-->';
