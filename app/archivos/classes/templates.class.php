...
class dle_template {
	...
	function load_template($tpl_name) {
		...
		if( $tpl_name == '' || ! file_exists( $this->dir . DIRECTORY_SEPARATOR . $tpl_name ) ) {
			die( "Falta el siguiente archivo: " . $tpl_name );
			return false;
		}
	...
	function load_file( $name, $include_file = "tpl" ) {
		...
		if ($include_file == "php") {
			...
			if ($type != "php") return "No se puede conectar con las siguientes extensiones: .tpl or .php";
			...
			if ( stristr ( dirname ($url['path']) , "subidas" ) !== false )
				return "El directorio para los archivos /subidas/ esta denegado";

			if ( stristr ( dirname ($url['path']) , "tema" ) !== false )
				return "El directorio para los archivos /tema/ esta denegado";

			if ($chmod_value == 777 ) return "Archivos {$url['path']} que estan en esta carpeta, no tiene permitido la escritura (CHMOD 777). Para que el sistema de CAO se conecte correctamente le recomendamos que cambie los parametros de seguridad.";

			if ( !file_exists($file_path."/".$file_name) ) return "Archivo {$url['path']} no encontrado.";
	...
	function sub_load_template( $tpl_name ) {
		...
		if( $tpl_name == '' || ! file_exists( $this->dir . DIRECTORY_SEPARATOR . $tpl_name ) ) {
			return "Falta el siguiente archivo: " . $tpl_name ;
			return false;
		}
