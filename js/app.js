...
nuevoPage: function(page) {
    ...
    $("#geocomplete").geocomplete({
        ...
        location: [storage.getItem('latitud_drink2nite'), storage.getItem('longitud_drink2nite')]
      });

      $.ajax({ url:'https://maps.googleapis.com/maps/api/geocode/json?latlng='+storage.getItem('latitud_drink2nite')+','+storage.getItem('longitud_drink2nite')+'&sensor=false&key=AIzaSyDHpSkI3_q35SfKPOOguHSjd5zm4D2QwNw',
         success: function(data){
             console.log(data);
             $('#geocomplete').val(data.results[1].formatted_address);
             $("input[name=formatted_address]").val(data.results[1].formatted_address);
             $("input[name=id]").val(data.results[1].place_id);
         }
});

    $("input[name=lat]").val(storage.getItem('latitud_drink2nite'));
    $("input[name=lng]").val(storage.getItem('longitud_drink2nite'));
